...
db.define_table('comic',
                Field('box', db.box, required=True, notnull=True, requires=IS_IN_DB(db, 'box.id', '%(name)s', zero='Choose a box')),
                ...
                Field('cover_image', 'upload', required=True, notnull=True, requires=[IS_IMAGE()])
                )
